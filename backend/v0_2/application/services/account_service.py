...
class AccountApplicationService:
    ...
    async def get_account(self, account_id: str = "default") -> Optional[AccountAggregate]:
        """Obtener información de cuenta"""
        return await self.account_repository.get_account(account_id)

    async def ensure_account_exists(self, account_id: str = "default") -> AccountAggregate:
    ...
    async def update_account_pricing(self, account: AccountAggregate) -> None:
        ...
        if doge_balance:
            try:
                doge_price_usdt = await self.market_data_pricer.get_price_usdt("DOGEUSDT")
        ...
        total_balance, current_balance = await self.balance_calculator.calculate_balances(
            account, prices
        )
