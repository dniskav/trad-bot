...
class StrategyApplicationService:
    ...
    async def create_strategy(
        ...
        if errors:
            await self.event_publisher.publish_strategy_event(
                config.name, f"strategy_config_error: {'; '.join(errors)}"
            )
            return None
    ...
    async def start_strategy(self, strategy_id: str) -> bool:
        ...
        if success:
            ...
            # Publicar evento
            await self.event_publisher.publish_strategy_event(
                strategy_id, "strategy_started", {"symbol": strategy.config.symbol}
            )
    ...
    async def stop_strategy(self, strategy_id: str) -> bool:
        ...
        if success:
            ...
            # Publicar evento
            await self.event_publisher.publish_strategy_event(
                strategy_id, "strategy_stopped", {"symbol": strategy.config.symbol}
            )
    ...
    async def generate_signal(
        self, strategy_id: str, market_data: Dict[str, Any]
        ...
        signal_result = await self.signal_evaluator.evaluate_signals(
            strategy.config.signals, indicators_data, market_data
        )
        ...
        if strategy.config.risk_management.enabled:
            ...
            if (
                account_balance
                and account_balance.amount < Money.from_float(10.0).amount
            ):
                await self.event_publisher.publish_strategy_event(
                    strategy_id,
            ...
            signal_result.signal = await self.risk_manager.apply_risk_management(
                signal_result.signal, account_balance.amount if account_balance else 0
            )
        ...
        if signal_result.signal:
            ...
            await self.performance_tracker.record_signal_generated(
                strategy_id, signal_result.signal
            )
    ...
    async def update_strategy_config(
        self, strategy_id: str, new_config: StrategyConfig
    ) -> bool:
    ...
    async def get_strategy_performance(self, strategy_id: str) -> Dict[str, Any]:
        ...
        return {**base_summary, **performance_data, "is_healthy": strategy.is_healthy()}

    async def evaluate_strategy_safety(self, strategy_id: str) -> Dict[str, Any]:
        ...
        safety_report = {"safe": True, "reasons": [], "warnings": []}

        # Verificar estado actual
        if strategy.status == StrategyStatus.ERROR:
            safety_report["safe"] = False
            safety_report["reasons"].append(
    ...
    async def get_account_balance_for_strategy(
