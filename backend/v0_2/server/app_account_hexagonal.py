...
account_router.dependency_overrides.setdefault(
    "account_service", inject_account_service
)
...
async def health_check_endpoint():
    ...
    try:
        ...
        account_status = health_data.get("account_integration", {}).get(
            "status", "unknown"
        )
...
async def integration_status():
    ...
    try:
        ...
        return {
            ...
            "legacy_fallback": integration_status.get(
                "legacy_fallback_available", False
            ),
            "services": {
                "hexagonal": (
                    "AccountApplicationService + Adapters"
                    if integration_status.get("hexagonal_available")
                    else "Not available"
                ),
                "legacy": (
                    "STMService (Fallback)"
                    if integration_status.get("legacy_fallback_available")
                    else "Not available"
                ),
            },
