...
from ..infrastructure.adapters.domain.strategy_service_adapter import StrategyServiceAdapter
...
class StrategyServiceIntegration:
    ...
    async def initialize(self, binance_service: BinanceService):
        ...
        try:
            ...
            self.strategy_application_service = await self.container.resolve_service(StrategyApplicationService)
            
            if not self.strategy_application_service:
                raise Exception("Failed to resolve StrategyApplicationService from DI Container")
            
            print(f"✅ StrategyApplicationService resolved: {type(self.strategy_application_service).__name__}")
            
            # Crear adapter que conecta con el router legacy
            self.strategy_adapter = StrategyServiceAdapter(self.strategy_application_service)
    ...
    async def _load_initial_strategies(self):
        ...
        try:
            ...
            print(f"📊 Strategy Loading Summary: {loaded_count}/{len(auto_load_strategies)} strategies loaded")
