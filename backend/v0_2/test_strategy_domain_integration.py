...
from domain.models.strategy import StrategyConfig, IndicatorConfig, SignalConfig, SignalCondition, IndicatorType, SignalType
...
async def test_strategy_domain_integration():
    ...
    try:
        ...
        perf_data = await strategy_service.get_strategy_performance(new_strategy.strategy_id)
        ...
        print(f"   Active strategies: {summary.get('global_metrics', {}).get('active_strategies', 0)}")
...
async def test_strategy_services_individually():
    ...
    try:
        ...
        from infrastructure.adapters.domain.signal_evaluator_service import SignalEvaluatorService
...
async def main():
    ...
    print(f"   Individual Services Test: {'✅ PASSED' if individual_test_passed else '❌ FAILED'}")
