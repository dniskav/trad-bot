...
class FilePositionRepository(IPositionRepository):
    ...
    async def get_active_positions(self, symbol: Optional[str] = None) -> List[PositionAggregate]:
            ...
            for pos_dict in positions_data:
                ...
                if "created_at" in pos_dict and isinstance(pos_dict["created_at"], str):
                    try:
                        ...
                        datetime.fromisoformat(pos_dict["created_at"].replace('Z', '+00:00'))
                    except:
                ...
                if "updated_at" in pos_dict and isinstance(pos_dict["updated_at"], str):
                    try:
                        from datetime import datetime
                        datetime.fromisoformat(pos_dict["updated_at"].replace('Z', '+00:00'))
                    except:
...
if __name__ == "__main__":
    ...
    async def test_repository():
        ...
        # Test crear posición
        from ...domain.models.position import PositionAggregate, OrderSide, Quantity, Price, Money
