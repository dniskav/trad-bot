...
class FileOrderRepository(IOrderRepository):
    ...
    async def get_orders_by_position(self, position_id: str) -> List[OrderAggregate]:
        ...
        try:
            ...
            raise Exception(
                f"Error getting orders for position {position_id}: {str(e)}"
            )
    ...
    def _load_orders(self) -> List[dict]:
        ...
        try:
            ...
            for order_dict in orders_data:
                if "created_at" in order_dict and isinstance(
                    order_dict["created_at"], str
                ):
                    try:
                ...
                if "updated_at" in order_dict and isinstance(
                    order_dict["updated_at"], str
                ):
                    try:
                        from datetime import datetime

                        datetime.fromisoformat(
                            order_dict["updated_at"].replace("Z", "+00:00")
                        )
                    except:
...
if __name__ == "__main__":
    ...
    async def test_order_repository():
        ...
        if retrieved:
            print(
                f"✅ Order retrieved: {retrieved.symbol} {retrieved.order_type.value}"
            )
        else:
