...
class StrategyServiceAdapter:
    ...
    async def get_all_strategies(self) -> Dict[str, StrategyInstance]:
        ...
        try:
            ...
            all_strategies = await self.strategy_service.strategy_repository.get_all_strategies()
    ...
    async def get_strategy(self, strategy_name: str) -> Optional[StrategyInstance]:
        ...
        try:
            strategy = await self.strategy_service.strategy_repository.get_strategy_by_name(strategy_name)
            if strategy:
                return strategy
        except Exception as e:
    ...
    async def get_strategy_performance(self, strategy_name: str) -> Optional[Dict[str, Any]]:
        ...
        return await self.strategy_service.get_strategy_performance(strategy.strategy_id)
    ...
    async def load_strategy(self, strategy_name: str) -> bool:
        ...
        try:
            ...
            config_file_path = f"backend/v0_2/server/strategies/configs/{strategy_name}.json"
            strategy_config = self.strategy_service.strategy_repository.load_strategy_from_config_file(config_file_path)
            ...
            await self.strategy_service.strategy_repository.save_strategy(strategy_instance)
    ...
    async def unload_strategy(self, strategy_name: str) -> bool:
        ...
        try:
            ...
            await self.strategy_service.strategy_repository.delete_strategy(strategy.strategy_id)
    ...
    def _get_strategy_sync(self, strategy_name: str) -> Optional[StrategyInstance]:
        ...
        try:
            ...
            if os.path.exists(config_file_path):
                ...
                config = self.strategy_service.strategy_repository.load_strategy_from_config_file(config_file_path)
                if config:
                    strategy_instance = StrategyInstance(
    ...
    async def create_strategy_from_config(self, strategy_config: StrategyConfig) -> Optional[str]:
        """Crear nueva estrategia desde configuración"""
        
        try:
            strategy_instance = await self.strategy_service.create_strategy(strategy_config)
            if strategy_instance:
                # Agregar a cache para compatibilidad
                self._loaded_strategies_cache[strategy_instance.config.name] = strategy_instance
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "healthy_strategies": sum(1 for h in health_report.values() if h.get("healthy", False)),
