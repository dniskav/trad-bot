...
class SimpleMarketDataPricer:
        ...
        for symbol in symbols:
            try:
                ...
                prices[symbol.upper()] = self.default_prices.get(
                    AssetType.DOGE if "DOGE" in symbol.upper() else AssetType.USDT,
                    1.0
                )
    ...
    async def calculate_conversion_rate(
        ...
        if from_price_usdt > 0:
            conversion_rate = Decimal(str(to_price_usdt)) / Decimal(str(from_price_usdt))
        else:
...
class BinanceMarketDataPricer(SimpleMarketDataPricer):
    ...
    async def get_multiple_prices_usdt(self, symbols: list[str]) -> Dict[str, float]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, params={"symbols": symbols_param}, timeout=self.timeout) as response:
                    if response.status == 200:
...
class AdvancedMarketDataPricer(BinanceMarketDataPricer):
    ...
    async def calculate_conversion_rate(
        ...
        try:
            ...
            from_price = prices.get(from_symbol.upper(), self.default_prices.get(from_asset, 1.0))
            to_price = prices.get(to_symbol.upper(), self.default_prices.get(to_asset, 1.0))
            ...
            self.conversion_cache[reverse_cache_key] = Decimal("1") / conversion_rate if conversion_rate > 0 else Decimal("1")
