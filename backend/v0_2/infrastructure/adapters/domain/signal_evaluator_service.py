...
class SignalEvaluatorService:
    ...
    async def _evaluate_single_signal(
        ...
        try:
            ...
            for condition in signal_config.conditions:
                ...
                condition_key = f"{condition.indicator}_{condition.operator}_{condition.value}"
            ...
            if signal_config.logic_type == "AND":
                all_conditions_met = len(conditions_evaluated) > 0 and all(conditions_evaluated.values())
            elif signal_config.logic_type == "OR":
                all_conditions_met = len(conditions_evaluated) > 0 and any(conditions_evaluated.values())
            else:
                all_conditions_met = len(conditions_evaluated) > 0 and all(conditions_evaluated.values())
            ...
            if confidence < signal_config.min_confidence:
                errors.append(f"Confidence {confidence:.2f} below minimum {signal_config.min_confidence}")
                return SignalGenerationResult(
                    signal=None,
                    indicators_data=indicators_data,
                    conditions_evaluated=conditions_evaluated,
    ...
    async def _evaluate_condition(
        ...
        try:
            ...
            return {
                "met": False,
                "reasoning": f"Error evaluating condition: {str(e)}"
            }

    def _extract_indicator_value(self, indicator_data: Dict[str, Any]) -> Optional[float]:
    ...
    async def _calculate_confidence(
        ...
        base_confidence = met_conditions / total_conditions if total_conditions > 0 else 0
    ...
    async def _create_signal(
        ...
        current_price = market_data.get("current_price", market_data.get("price", 100.0))
...
class AdvancedSignalEvaluatorService(SignalEvaluatorService):
    ...
    def get_success_rate(self, signal_name: str = None) -> Dict[str, float]:
        ...
        success_rate = successful_signals / total_evaluations if total_evaluations > 0 else 0
