...
class STMTradingExecutor(ITradingExecutor):
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200 and response_data.get("success", False):
                        log.info(f"✅ Market order executed: {response_data.get('orderId')}")
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200 and response_data.get("success", False):
                        log.info(f"✅ Limit order placed: {response_data.get('orderId')}")
    ...
    async def execute_stop_order(
        ...
        try:
            log.info(f"Executing stop order: {side} {quantity} {symbol} stop@{stop_price}")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200 and response_data.get("success", False):
                        log.info(f"✅ Stop order placed: {response_data.get('orderId')}")
