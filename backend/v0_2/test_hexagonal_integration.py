...
async def test_hexagonal_integration():
    ...
    try:
        ...
        if strategy_service:
            print(f"✅ StrategyApplicationService resolved: {type(strategy_service).__name__}")
        else:
...
async def test_individual_services():
    ...
    try:
        ...
        sma_result = await indicator_service.calculate_indicator("SMA", {"period": 5}, market_data)
        print(f"✅ SMA Result: {sma_result}")
        
        rsi_result = await indicator_service.calculate_indicator("RSI", {"period": 14}, market_data)
