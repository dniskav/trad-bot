...
class WebSocketService:
    ...
    async def connect(
        ...
        try:
            ...
            if not client_id:
                client_id = (
                    f"client_{len(self.connections)}_{int(datetime.now().timestamp())}"
                )
            ...
            log.info(
                f"WS client connected: {client_id} (total: {len(self.connections)})"
            )
    ...
    async def disconnect(self, client_id: str) -> None:
        ...
        try:
            if client_id in self.connection_metadata:
                ...
                log.info(
                    f"WS client disconnected: {client_id} (total: {len(self.connections)})"
                )
    ...
    async def broadcast(
        self, message: Dict[str, Any], client_filter: Optional[List[str]] = None
    ) -> None:
        """
            ...
            if client_id in self.connection_metadata:
                ...
                if websocket in self.connections:
                    try:
                        ...
                        log.warning(
                            f"Error sending to client {client_id}: {type(e).__name__}: {e}"
                        )
        ...
        log.debug(
            f"Broadcast completed: {successful_sends} sent, {len(disconnected_clients)} disconnected"
        )
    ...
    async def get_service_status(self) -> Dict[str, Any]:
        ...
        total_messages = sum(
            m["message_count"] for m in self.connection_metadata.values()
        )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            is_healthy = (
                status_response["active_connections"] is not None
                and status_response["registered_clients"] is not None
