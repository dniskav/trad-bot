...
class RealTradingManager:
    ...
    def __init__(self):
        ...
        self.max_concurrent_positions_per_bot = int(
            os.getenv("MAX_CONCURRENT_POSITIONS_PER_BOT", "5")
        )

        # Configuración de apalancamiento
        self.leverage = int(os.getenv("LEVERAGE", "3"))  # Apalancamiento 3x por defecto
        self.margin_type = os.getenv(
            "MARGIN_TYPE", "CROSSED"
        )  # Cross Margin por defecto
    ...
    def initialize_bot_status_from_tracker(self, trading_tracker):
        ...
        if trading_tracker and hasattr(trading_tracker, "bot_status"):
            ...
            logger.info(
                f"🤖 Estado de bots inicializado desde archivo: Conservative={self.bot_status['conservative']}, Aggressive={self.bot_status['aggressive']}"
            )
        else:
    ...
    def initialize_active_positions_from_tracker(self, trading_tracker):
        ...
        if trading_tracker and hasattr(trading_tracker, "get_active_positions"):
            ...
            logger.info(
                f"📊 Posiciones activas cargadas desde archivo: Conservative={conservative_count}, Aggressive={aggressive_count}"
            )
        else:
    ...
    def sync_active_positions_with_tracker(self, trading_tracker):
        ...
        if trading_tracker and hasattr(trading_tracker, "active_positions"):
            ...
            logger.info(
                f"📊 Active positions antes: {list(trading_tracker.active_positions.keys())}"
            )
            ...
            logger.info(
                f"📊 Active positions después de sync: {list(trading_tracker.active_positions.keys())}"
            )
            ...
            logger.info(
                f"📊 Active positions final: {list(trading_tracker.active_positions.keys())}"
            )
            # HABILITADO PARA PRUEBA
            logger.info("✅ save_history() HABILITADO PARA PRUEBA")
            trading_tracker.save_history()
            logger.info(
                "📊 Posiciones activas sincronizadas con el tracker (preservando bots plug and play)"
            )
    ...
    def sync_with_binance_orders(self, trading_tracker=None):
        ...
        try:
            ...
            for bot_type in ["conservative", "aggressive"]:
                for position_id, position_data in self.active_positions[
                    bot_type
                ].items():
                    ...
                    if order_id and order_id not in binance_order_ids:
                        ...
                        logger.warning(
                            f"⚠️ Posición {position_id} ya no existe en Binance, removiendo"
                        )
                ...
                if trading_tracker and order_id:
                    try:
                        ...
                        if current_price:
                            ...
                            trade_value = current_price * position_data.get(
                                "quantity", 0
                            )
                            ...
                            logger.info(
                                f"🔒 Orden {order_id} cerrada en historial (posición removida de Binance)"
                            )
                    except Exception as e:
                        logger.error(
                            f"❌ Error cerrando orden {order_id} en historial: {e}"
                        )
            ...
            if positions_to_remove:
                logger.info(
                    f"🔄 {len(positions_to_remove)} posiciones obsoletas removidas"
                )
                ...
                logger.info(
                    "✅ Todas las posiciones activas están sincronizadas con Binance"
                )
    ...
    def sync_history_with_binance_orders(self, trading_tracker=None):
        ...
        if not self.client or not trading_tracker:
            logger.warning(
                "⚠️ Cliente de Binance o TradingTracker no disponible para sincronización del historial"
            )
            return
        ...
        try:
            ...
            for order in history_open_orders:
                ...
                if order_id and order_id not in binance_order_ids:
                    ...
                    logger.warning(
                        f"⚠️ Orden {order_id} ya no existe en Binance, cerrando en historial"
                    )
            ...
            for order in orders_to_close:
                ...
                try:
                    ...
                    if current_price:
                        ...
                        logger.info(
                            f"🔒 Orden {order_id} cerrada en historial (no existe en Binance)"
                        )
                        ...
                        if (
                            position_id
                            and bot_type
                            and bot_type in self.active_positions
                        ):
                            if position_id in self.active_positions[bot_type]:
                                del self.active_positions[bot_type][position_id]
                                logger.info(
                                    f"🗑️ Posición {position_id} removida de posiciones activas"
                                )
            ...
            if orders_to_close:
                logger.info(
                    f"🔄 {len(orders_to_close)} órdenes del historial cerradas automáticamente"
                )
    ...
    def update_all_orders_status(self, trading_tracker=None):
        ...
        try:
            ...
            if open_orders:
                logger.info(
                    f"📊 {len(open_orders)} órdenes abiertas actualizadas con precio ${current_price}"
                )
        ...
        try:
            ...
            for order in open_orders:
                ...
                stop_loss, take_profit = (
                    trading_tracker.calculate_stop_loss_take_profit(
                ...
                if should_close:
                    if is_synthetic:
                        logger.info(
                            f"🧪 {bot_type.upper()} - Cerrando posición sintética por {close_reason}: ${entry_price:.5f} → ${current_price:.5f} ({pnl_pct:.2f}%)"
                        )
                    else:
                        logger.info(
                            f"🎯 {bot_type.upper()} - Cerrando posición por {close_reason}: ${entry_price:.5f} → ${current_price:.5f} ({pnl_pct:.2f}%)"
                        )
    ...
    def close_position_with_tracking(
        self, bot_type: str, position_id: str, trading_tracker=None
    ):
        """Cierra una posición y actualiza el tracking en el historial"""
        if (
            bot_type not in self.active_positions
            or position_id not in self.active_positions[bot_type]
        ):
        ...
        try:
            ...
            logger.info(
                f"🔒 Posición cerrada: {bot_type.upper()} {position_id} a ${close_price}"
            )
            return True
    ...
    def check_risk_limits(self, trade_amount: float, bot_type: str) -> Dict[str, Any]:
        ...
        if self.daily_pnl <= -self.max_daily_loss:
            ...
            checks["reasons"].append(
                f"Límite de pérdida diaria alcanzado: ${self.daily_pnl:.2f}"
            )
        ...
        position_tolerance = (
            0.001  # 0.1 centavos de tolerancia para errores de precisión
        )
        ...
        if trade_amount > max_position_size_with_leverage + position_tolerance:
            ...
            checks["reasons"].append(
                f"Tamaño de posición excede límite: ${trade_amount:.6f} > ${max_position_size_with_leverage:.6f} (diferencia: ${difference:.6f})"
            )
        ...
        total_max_positions = (
            self.max_concurrent_positions_per_bot * 2
        )  # Total para ambos bots
        ...
        if bot_positions >= dynamic_limit:
            ...
            if active_bots == 1:
                checks["reasons"].append(
                    f"Máximo de posiciones concurrentes alcanzado para {bot_type}: {bot_positions}/{dynamic_limit} (usando todas las posiciones disponibles)"
                )
            else:
                checks["reasons"].append(
                    f"Máximo de posiciones concurrentes alcanzado para {bot_type}: {bot_positions}/{dynamic_limit}"
                )
    ...
    def check_balance_availability(
        self, signal: str, bot_type: str, current_price: float
    ) -> Dict[str, Any]:
        ...
        if self.leverage > 1:
            ...
            logger.info(
                f"💰 Balance disponible (Margin): {usdt_balance:.2f} USDT, {doge_balance:.2f} DOGE"
            )
            logger.info(
                f"⚡ Apalancamiento: {self.leverage}x - Poder de trading: ${usdt_balance * self.leverage:.2f} USDT"
            )
        else:
            ...
            logger.info(
                f"💰 Balance disponible (Spot): {usdt_balance:.2f} USDT, {doge_balance:.2f} DOGE"
            )
        ...
        if signal == "BUY":
            ...
            if bot_type == "conservative":
                ...
                required_usdt = min(
                    base_usdt * leverage_multiplier, max_position_with_leverage
                )
            else:  # aggressive
            ...
            if bot_type == "conservative":
                base_doge = min(
                    max(min_doge_for_profit, doge_balance * 0.1), max_doge_value
                )
                required_doge = min(base_doge * leverage_multiplier, max_doge_value)
            else:  # aggressive
                base_doge = min(
                    max(min_doge_for_profit, doge_balance * 0.15), max_doge_value
                )
                required_doge = min(base_doge * leverage_multiplier, max_doge_value)
            ...
            if doge_balance < required_doge:
                available_doge = max(
                    1.0, doge_balance * 0.9
                )  # Usar 90% del disponible, mínimo 1.0 DOGE
    ...
    def place_order(
        self, bot_type: str, signal: str, current_price: float, trading_tracker=None
    ) -> Dict[str, Any]:
        ...
        if not self.is_bot_active(bot_type):
            logger.info(
                f"ℹ️ {bot_type.upper()} - Bot desactivado, ignorando señal: {signal}"
            )
            return {
                "success": False,
        ...
        if signal == "BUY":
            ...
            quantity_usdt = balance_check.get(
                "available_usdt", balance_check["required_usdt"]
            )
            quantity_doge = quantity_usdt / current_price

        else:  # SELL
            # Usar la cantidad ajustada si fue necesario
            quantity_doge = balance_check.get(
                "available_doge", balance_check["required_doge"]
            )
        ...
        if quantity_doge < 4.0:
            logger.warning(
                f"⚠️ {bot_type.upper()} - Cantidad muy pequeña: {quantity_doge} DOGE, usando mínimo 4 DOGE"
            )
            quantity_doge = 4.0

        # Ejecutar trade
        result = self.execute_trade(
            symbol, signal, current_price, quantity_doge, bot_type, trading_tracker
        )

        if result["success"]:
            logger.info(
                f"✅ {bot_type.upper()} - Trade ejecutado: {signal} {quantity_doge} DOGE a ${current_price}"
            )
        else:
            logger.error(
                f"❌ {bot_type.upper()} - Error en trade: {result.get('error', 'Error desconocido')}"
            )
    ...
    def execute_trade(
        ...
        current_price: float,
        quantity: float = None,
        bot_type: str = "conservative",
        trading_tracker=None,
    ) -> Dict[str, Any]:
        """Ejecuta un trade basado en la señal del bot"""

        if signal not in ["BUY", "SELL"]:
            return {"success": False, "error": f"Señal inválida: {signal}"}
        ...
        if result["success"]:
            ...
            position_id = (
                f"{symbol}_{bot_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
    ...
    def place_order_raw(
        ...
        if not self.client:
            return {"success": False, "error": "Cliente de Binance no inicializado"}
        ...
        try:
            ...
            if notional_filter:
                ...
                if current_notional < min_notional:
                    ...
                    logger.info(
                        f"📊 Ajustando cantidad para cumplir NOTIONAL mínimo: {quantity} {symbol}"
                    )
            ...
            if self.trading_mode == "real" and self.leverage > 1:
                ...
                logger.info(
                    f"⚡ Orden de margen ejecutada ({self.leverage}x): {side} {quantity} {symbol}"
                )
            else:
                ...
                logger.info(
                    f"   Precio: ${order.get('fills', [{}])[0].get('price', 'N/A')}"
                )
    ...
    def close_position(self, position_id: str, bot_type: str) -> Dict[str, Any]:
        ...
        result = self.place_order_raw(
            position["symbol"], close_side, position["quantity"], bot_type
        )
        ...
        if result["success"]:
            ...
            exit_price = float(
                result["order"]
                .get("fills", [{}])[0]
                .get("price", position["entry_price"])
            )
    ...
    def get_trading_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "risk_status": (
                "OK" if self.daily_pnl > -self.max_daily_loss else "LIMIT_REACHED"
            ),
    ...
    def deactivate_bot(self, bot_type: str, trading_tracker=None) -> bool:
        ...
        if positions_to_close:
            logger.info(
                f"🔄 Cerrando {len(positions_to_close)} posiciones del bot {bot_type.upper()}..."
            )
    ...
    def get_margin_level(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "is_safe": margin_level
                > 2.0,  # Consideramos seguro si margin level > 2.0
            }
        except Exception as e:
    ...
    def check_margin_safety(self) -> bool:
        ...
        if not is_safe:
            logger.warning(
                f"⚠️ Margin level bajo: {margin_level:.2f} (límite seguro: 2.0)"
            )
