...
class OrderAggregate:
    ...
    def __post_init__(self):
        ...
        if (
            self.order_type in [OrderType.STOP_MARKET, OrderType.STOP_LOSS]
            and not self.stop_price
        ):
            raise ValueError("STOP orders must have a stop_price")

    def execute(
        self, execution_price: Price, executed_qty: Quantity, commission: Money
    ) -> None:
        """Ejecutar orden"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "executed_price": (
                str(self.executed_price.value) if self.executed_price else None
            ),
            "executed_quantity": (
                str(self.executed_quantity.amount) if self.executed_quantity else None
            ),
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "OrderAggregate":
        ...
        if data.get("executed_price"):
            executed_price = Price.from_float(
                float(data["executed_price"]), data["symbol"]
            )
