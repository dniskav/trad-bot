...
class TradingTracker:
    ...
    def __init__(self, binance_client=None):
        # Cambiar a múltiples posiciones por bot (compatible con RealTradingManager)
        self.positions = {
    ...
    def _calculate_initial_balance_from_binance(self, binance_client):
        ...
        try:
            ...
            if ada_balance > 0:
                ...
                ada_value_usdt = ada_balance * ada_price
            else:
                ada_value_usdt = 0.0
            
            total_balance = usdt_balance + ada_value_usdt
            
            logger.info(f"💰 Balance inicial calculado desde Binance:")
            logger.info(f"   USDT: ${usdt_balance:.2f}")
            logger.info(f"   ADA: {ada_balance:.2f} (${ada_value_usdt:.2f})")
    ...
    def calculate_stop_loss_take_profit(self, bot_type: str, signal: str, entry_price: float):
        ...
        # Calcular comisiones totales (entrada + salida)
        total_fee_rate = self.total_fee_rate * 2  # Entrada + salida
        
        # Ajustar take profit para que supere las comisiones y genere utilidad mínima
        min_profit_pct = total_fee_rate + 0.001  # Comisiones + 0.1% de utilidad mínima
        adjusted_take_profit = max(config['take_profit'], min_profit_pct)
        ...
        logger.info(f"📊 {bot_type.upper()} - SL/TP calculados:")
        logger.info(f"   Comisiones totales: {total_fee_rate*100:.3f}%")
        logger.info(f"   Take Profit ajustado: {adjusted_take_profit*100:.3f}% (mínimo: {min_profit_pct*100:.3f}%)")
    ...
    def load_history(self):
        ...
        try:
            ...
            # Solo cargar balance si hay historial de trading
            if self.position_history:
                ...
                # Si no hay historial, mantener el balance calculado desde Binance
                logger.info(f"💰 Sin historial, usando balance desde Binance: ${self.initial_balance:.2f}")
    ...
    def update_position(self, bot_type: str, signal: str, current_price: float, quantity: float = 1.0):
        ...
        # Si el bot cambió a HOLD y tenemos posición abierta
            ...
            # Solo cerrar si es rentable después de comisiones
            if position['pnl_net'] > 0:
                position['close_reason'] = 'Señal Contraria (Rentable)'
                
                logger.info(f"🔒 {bot_type.upper()} - Cerrando posición rentable:")
                ...
                logger.info(f"⚠️ {bot_type.upper()} - Manteniendo posición (no rentable):")
                logger.info(f"   PnL Bruto: ${position['pnl']:.4f} ({position['pnl_pct']:.2f}%)")
                logger.info(f"   Comisiones: ${total_fees:.4f}")
                logger.info(f"   PnL Neto: ${position['pnl_net']:.4f} ({position['pnl_net_pct']:.2f}%)")
                logger.info(f"   Esperando Take Profit o Stop Loss...")
                
                # No cerrar la posición, mantenerla abierta
                pass
...
def initialize_trading_tracker(binance_client=None):
    """Inicializa el trading tracker con el cliente de Binance"""
