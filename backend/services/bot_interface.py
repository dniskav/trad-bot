...
class BaseBot(ABC):
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'synthetic_balance': self.synthetic_balance if self.config.synthetic_mode else None,
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'active_positions': len([p for p in self.positions if p.get('status') == 'open']),
            ...
            'last_signal_time': self.last_signal.metadata.get('timestamp') if self.last_signal else None
        }
    
    def open_synthetic_position(self, signal: TradingSignal, current_price: float) -> Dict[str, Any]:
        """
        ...
        self.logger.info(f"📊 Posición sintética abierta: {signal.signal_type.value} a ${current_price:.5f}")
    ...
    def check_synthetic_positions(self, current_price: float) -> List[Dict[str, Any]]:
        ...
        for position in self.synthetic_positions[:]:  # Copia para iterar
            ...
            if should_close:
                ...
                self.logger.info(f"🔒 Posición sintética cerrada: {close_reason} - PnL: ${pnl:.2f}")
